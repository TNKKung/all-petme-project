...
function Popup2({ popup1, popup2, popupFinish, setType }) {
  ...
  const [form, setForm] = useState({
    ...
    question1: 'สุนัขพันธ์นี้ชอบการเอาใจใส่ คุณมีเวลาเลี้ยงดูได้ไหม?',
    question2: 'สุนัขพันธ์นี้ใช้ค่าใช้จ่ายในการเลี้ยงดูสูง คุณดูแลได้ไหม',
    question3: null,
    question4: null,
    question5: null,
    picture: null,
  ...
  const sendDataImage = async (e) => {
    ...
    const res = await fetch("https://api2.evera.cloud/uploadFile", {
      method: "POST",
      body: formData,
    })
      .then((res) => res.text())
